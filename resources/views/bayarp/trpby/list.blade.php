                                        ...
                                        @foreach ($payments as $payment)
                                            <tr>
                                                <td><button class="btn btn-primary btn-sm mb-0"
                                                        onclick="window.location='{{ url($url_menu . '/show/' . encrypt($payment->id)) }}'">View</button>
                                                </td>
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ $payment->nomor_bukti }}</td>
                                                <td>{{ $payment->tanggal_pembayaran }}</td>
                                                <td>{{ $payment->pemasok }}</td>
                                                <td>{{ number_format($payment->nilai_pembayaran, 2, ',', '.') }}</td>
