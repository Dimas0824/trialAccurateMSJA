                                ...
                                <table class="table display" id="list_{{ $dmenu }}">
                                    ...
                                    <tbody>
                                        @foreach ($orders as $order)
                                            <tr
                                                {{ $order->status === 'dibatalkan' ? 'style=background-color:#ffe9ed;' : '' }}>
                                                    ...
                                                    <div class="btn-group"><button class="btn btn-primary btn-sm mb-0 px-3"
                                                        ...
                                                        <ul class="dropdown-menu">
                                                            @if ($authorize->edit == '1' && $order->status === 'draf')
                                                                ...
                                                                @endif @if ($authorize->delete == '1' && !in_array($order->status, ['selesai', 'dibatalkan']))
                                                                    <li>
                                                                        <hr class="dropdown-divider">
                                                                    </li>
                                                                    <li>
                                                                        <form
                                                                            action="{{ url($url_menu . '/' . encrypt($order->id)) }}"
                                                                            method="POST">@csrf @method('DELETE')<button
                                                                                class="btn btn-sm btn-danger mx-2 mb-0 w-90"
                                                                                onclick="return confirm('Batalkan pesanan ini?')"><i
                                                                                    class="fas fa-ban"></i><span
                                                                                    class="font-weight-bold">
                                                                                    Batalkan</span></button></form>
                                                                    </li>
                                                                @endif
                                                        </ul>
                                                    </div>
                                                ...
                                                <td class="text-sm font-weight-bold text-dark">{{ $order->nomor }}</td>
                                                <td class="text-sm">{{ $order->tanggal }}</td>
                                                <td class="text-sm">{{ $order->pemasok }}</td>
                                                <td class="text-sm">{{ $order->mata_uang }}</td>
                                                <td class="text-sm">{{ number_format($order->total_akhir, 2, ',', '.') }}
                                                </td>
                                                <td class="text-sm">{{ ucfirst($order->status) }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                        ...
                        <div class="row px-4 py-2">
                            <div class="col-lg">
                                <div class="nav-wrapper" id="noted"><code>Note : <i aria-hidden="true"
                                            style="color: #ffc2cd;" class="fas fa-circle"></i> Dokumen dibatalkan</code>
                                </div>
                            </div>
                        </div>
                    </div>
